...
> Derived from [problemStatement.md](file:///Users/shreyash/NextLeap/Groww_Pulse/docs/problemStatement.md) and [architecture.md](file:///Users/shreyash/NextLeap/Groww_Pulse/docs/architecture.md).
...
| **4** | Rendering & Delivery | Docs append + Gmail send via REST API POST | 3–4 days |
...
- [ ] Define dependency groups:
 - **Core:** `mcp`, `click`, `pyyaml`, `google-play-scraper`, `emoji`, `langdetect`
...
- [ ] Create the full directory tree per [architecture.md §9](file:///Users/shreyash/NextLeap/Groww_Pulse/docs/architecture.md#L458):
...
- [ ] Create `config.yaml` with all sections (product, ingestion, clustering, llm, delivery, receipts) using defaults from [architecture.md §8](file:///Users/shreyash/NextLeap/Groww_Pulse/docs/architecture.md#L419).
- [ ] Create `mcp_servers.json` with all three server declarations per [architecture.md §2](file:///Users/shreyash/NextLeap/Groww_Pulse/docs/architecture.md#L55).
...
- [ ] Create `mcp_servers/playstore_reviews/scraper.py`:
 - Wrap `google-play-scraper` library's `reviews()` function.
 - Accept parameters: `app_id`, `lang`, `count`, `sort_order`.
 - Return a list of `Review` dicts matching the schema from [architecture.md §3.2](file:///Users/shreyash/NextLeap/Groww_Pulse/docs/architecture.md#L118):
...
```
 ...
 - **Apply Custom Filters:** Filter out reviews that have fewer than 8 words, contain emojis, or are not in English (use `emoji` and `langdetect` libraries).
 - Handle pagination: the library returns reviews in batches; loop until `count` is met or no more results.


#### 1.2 Build MCP server


- [ ] Create `mcp_servers/playstore_reviews/server.py`:
...
- [ ] Create `mcp_servers/playstore_reviews/__main__.py`:
...
- [ ] Write `tests/test_scraper.py`:
 - Unit test: mock `google-play-scraper` responses, verify schema mapping and custom filters (drop < 8 words, emojis, non-English).
 - Integration test (marked `@pytest.mark.integration`): fetch 10 real reviews, validate schema.
- [ ] Manual smoke test: start server, call `fetch_reviews` via MCP inspector or a test client script.
...
- [ ] Implement `groww_pulse/ingestion.py`:
...
- [ ] Filter reviews where `timestamp` falls within the configured rolling window:
...
- [ ] Apply PII scrubbing to every review's `text` and `author` fields.


#### 2.4 Deduplication


- [ ] Deduplicate reviews by `review_id` (keep first occurrence).
...
- [x] Implement the embedding step in `groww_pulse/clustering.py`:
 - Accept `Review[]`, extract `text` field.
 - **Normalize Text:** Strip excess whitespace and lowercase text to optimize embedding quality.
 - Generate embeddings using `sentence-transformers` (model: `BAAI/bge-small-en-v1.5`).
...
- [ ] Apply UMAP to reduce embeddings to `config.clustering.umap_n_components` dimensions:
...
- [ ] Apply HDBSCAN on the UMAP-reduced embeddings:
 ...
 - Build `Cluster` objects per [architecture.md §3.4](file:///Users/shreyash/NextLeap/Groww_Pulse/docs/architecture.md#L174):
...
#### 3.5 LLM summarization (Groq: llama-3.3-70b-versatile)


- [ ] Implement `groww_pulse/summarizer.py` using Groq API:
 ...
 - Parse the LLM structured output (use JSON mode or function calling) into `ThemeSummary` objects per [architecture.md §3.5](file:///Users/shreyash/NextLeap/Groww_Pulse/docs/architecture.md#L196).
...
#### 3.6 Token budget enforcement (Groq Limits)


- [ ] Track cumulative token and request usage across all LLM calls in the run.
- [ ] Enforce Groq tier limits for `llama-3.3-70b-versatile`:
  - **Requests per minute:** 30
  - **Requests per day:** 1,000
  - **Tokens per minute:** 12,000
  - **Tokens per day:** 100,000
- [ ] If usage exceeds the minute limits, implement rate-limiting (sleep).
- [ ] If usage exceeds daily limits or configured `max_tokens_per_day`, stop processing remaining clusters and log a warning.
...
- [ ] Implement the Docs rendering path in `groww_pulse/renderer.py`:
 - Accept `ThemeSummary[]`, ISO week, config.
 - Generate the section structure per [architecture.md §3.6](file:///Users/shreyash/NextLeap/Groww_Pulse/docs/architecture.md#L237):
 ...
 - Output: structured plain text payload compatible with Railway Delivery API.


#### 4.2 Email renderer


- [ ] Implement the email rendering path in `groww_pulse/renderer.py`:
...
- [ ] Implement Docs delivery in `groww_pulse/delivery.py`:
 - Use the `requests` library to POST to the custom Railway Delivery API (`https://kharpreet-mcp-server-production-8e75.up.railway.app/append_to_doc`).
 - Pass the rendered plain text payload in the JSON body, including `doc_id` and `content`.
 - Capture the response: `doc_id`, `heading_id`, `revision_id`.
 - Handle errors: if the API is unreachable or returns a 4xx/5xx status, raise a clear exception (do not proceed to email).


#### 4.4 Gmail delivery


- [ ] Implement Gmail delivery in `groww_pulse/delivery.py`:
 - **Step 1:** Make a POST request to `/create_email_draft` using the rendered `EmailPayload` (formatting as `to`, `subject`, and `body`).
 - **Step 2:** If `--draft-only` is set, stop and log the draft ID.
 - **Step 3:** Otherwise, make a POST request to `/send_draft`. Capture `message_id`.


#### 4.5 Orchestration


- [ ] Wire the full pipeline in `groww_pulse/__main__.py`:
...
- [ ] `tests/test_delivery.py`:
 - Mock `requests.post` for Docs: verify correct payload.
 - Mock `requests.post` for Gmail: verify draft creation + send sequence.
 - Test `--draft-only` stops after draft.
 - Test error: Docs fails → email is not attempted.
...
flowchart LR
   ...
   B -- "Email payload" --> C
   C -- "HTTP POST: append_to_doc" --> R["Railway Delivery API"]
   C -- "HTTP POST: draft + send" --> R
```
...
- [ ] Implement `groww_pulse/receipts.py`:
 - Write a JSON receipt to `data/receipts/groww_{iso_week}.json` after each run per [architecture.md §6](file:///Users/shreyash/NextLeap/Groww_Pulse/docs/architecture.md#L347).
...
- [ ] Before appending to the Google Doc, check for an existing section:
...
- [ ] Handle the partial-failure scenario per [architecture.md §12](file:///Users/shreyash/NextLeap/Groww_Pulse/docs/architecture.md#L548):
...
- [ ] Add retry logic with exponential backoff for MCP calls:
 ...
 - Railway Delivery API: 3 retries for POST requests.
- [ ] Handle edge cases:
...
- [ ] All errors logged with structured context (ISO week, step, error message).
...
- [ ] `tests/test_error_handling.py`:
 ...
 - MCP unreachable → retries then fails with receipt.
...
**Goal:** Transform `ThemeSummary[]` into Google Docs content and email HTML, then deliver both via Railway Delivery API.
...
flowchart LR
   ...
   B -- "Docs payload" --> C["delivery.py"]
   B -- "Email payload" --> C
   C -- "MCP: append_doc" --> R["Railway Delivery MCP (SSE)"]
   C -- "MCP: draft + send" --> R
```
...
- [ ] Handle the partial-failure scenario per architecture.md §12:
 - Doc OK + Email failed: Re-run skips Doc (idempotent), retries email.
 - Doc failed: Re-run retries from Doc step.
 - Implementation: check receipt status for each delivery step before executing.
...
- [ ] Add retry logic with exponential backoff for MCP calls:
 - Play Store MCP: 3 retries, backoff 1s → 2s → 4s.
 - Railway Delivery MCP: 3 retries.
- [ ] Handle edge cases:
 - 0 reviews returned → abort with warning, no delivery.
 - 0 clusters → abort with warning, no delivery.
 - LLM quota exceeded → fail run, partial receipt.
...
graph TD
   ...
   style P4 fill:#0f3460,stroke:#533483,color:#eee
   style P5 fill:#0f3460,stroke:#533483,color:#eee
   style P6 fill:#22c55e,stroke:#003915,color:#000
```
...
- [x] **Phase 0:** `pyproject.toml`, directory structure, `config.yaml`, `mcp_servers.json`, CLI skeleton
- [x] **Phase 1:** `scraper.py`, `server.py`, `__main__.py` for Play Store MCP; scraper tests
- [x] **Phase 2:** `ingestion.py`, `pii.py`; MCP client integration; filtering + scrubbing tests
- [x] **Phase 3:** `clustering.py`, `summarizer.py`; embeddings → UMAP → HDBSCAN → LLM; quote validation; tests
- [x] **Phase 4:** `renderer.py`, `delivery.py`; Docs + Gmail MCP delivery; end-to-end test with `--draft-only`
- [x] **Phase 5:** `receipts.py`, idempotency logic, error handling, `status` command, all tests green
- [ ] **Phase 6:** Frontend Dashboard (Next.js/React); Glassmorphism UI; Review Explorer + Theme Drilldown

## Phase 6: Web Dashboard (Frontend)

**Goal:** Build a rich, interactive Next.js or React web dashboard to consume the JSON summaries (`data/phase3_output.json`) and raw reviews (`data/actual_reviews.json`) produced by the pipeline.

### Requirements:
- **Design:** Modern Corporate + Glassmorphism aesthetic as defined in `design/DESIGN.md` (Deep Navy `#020617` base, Neon Green `#4be277` accents).
- **Views:**
  1. **Weekly Pulse Dashboard:** High-level KPI cards, sentiment donut chart, and a grid of interactive Theme Cards.
  2. **Review Explorer:** Searchable/filterable data table of raw reviews with sort controls (by Thumbs Up, Rating, Date).
- **Interactivity:** Clicking a Theme Card in the dashboard filters the Review Explorer table to show only the reviews belonging to that cluster.
- **Tech Stack:** Next.js / Vite, TailwindCSS (for glassmorphism and layout), Recharts (data viz), Framer Motion (micro-animations).
